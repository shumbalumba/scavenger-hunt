...
const texts = ['LAURYNAI, ŽAIDIMAS NETRUKUS PRASIDĖS.', 'GEGUŽĖS 9D. 18:30 BŪK PASIRUOŠĘS.', 'STARTO VIETA NAMIE.', 'TAU REIKĖS:', ' ', '• Aprangos į gamtą ir į šventę atitinkančios oro sąlygas ', '• Bolt paspirtukas ar panaši transporto priemonė. Svarbu turėti prieigą prasidėjus žaidimui. Automobilis netinka.', '• Pakrautas telefonas', '• Popierius ir tušinukas', '• Alaus virimo proceso aprašas spausdintas ant lapo', '• Mėgstamas užkandis prie alaus', '• Rulonas šikpopierio', '• 5 - 8 prezervatyvai'];
...
export const Countdown = ({ targetDate }: { targetDate: Date }) => {
    ...
    if (glitchActive >= 0 && Math.random() < 0.15) {
      ...
      const secondaryStyle = (() => {
        switch (secondaryGlitch) {
          case 0: return { transform: 'translateX(-2px)' };
          case 1: return { opacity: 0.7 };
          case 2: return { filter: 'hue-rotate(90deg)' };
          case 3: return { transform: 'translateY(3px)' };
          case 4: return { transform: 'skewX(-10deg)' };
          case 5: return { filter: 'contrast(1.2)' };
          case 6: return { transform: 'rotate(-3deg)' };
          case 7: return { transform: 'scaleX(1.1)' };
          case 8: return { filter: 'brightness(1.3)' };
          default: return {};
        }
      })();
  ...
  return (
    <>
      <div onClick={handleClick} className={`bg-gray-950 p-2 lg:p-6 rounded-lg border-4 ${glitchActive > 5 ? 'border-red-600 border-dashed -skew-x-9' : 'border-gray-800'} mb-4 relative`}>
